...
class pages{
    ...
    private function first(){
        ...
        if($this->page==1){
            @$html.='<a class="icon-double-angle-left page_but font_size_16 hover" style="cursor:not-allowed;"></a>';
        }else{
            @$html.='<a href="?page=1'.$this->caijieary($this->arycont).'" class="icon-double-angle-left page_but font_size_16"></a>';
        }
        return $html;
    }
    //上一页
    private function prev(){
        if($this->page==1){
            ...
            @$html.='<a  href="?page='.($this->page-1).$this->caijieary($this->arycont).'" class="icon-angle-left page_but font_size_16"></a>';
        }
        return $html;
    }
    //下一页
    private function next(){
        if($this->page == $this->pagenum){
            ...
            @$html.='<a class="icon-angle-right page_but font_size_16" href="?page='.($this->page+1).$this->caijieary($this->arycont).'"></a>';
        }
        return $html;
    }
    //尾页
    private function last(){
        if($this->page==$this->pagenum){
            ...
            @$html.='<a class="icon-double-angle-right page_but font_size_16" href="?page='.($this->pagenum).$this->caijieary($this->arycont).'"></a>';
        }
        return $html;
    }
    //当前页
        ...
        for($i=1;$i<=5;$i++){
            ...
            if($page > 0 && $page != $this->page){
                if($page <= $this->pagenum){
                    $linkpag .= '<a class="page_but" href="?page=' . $page . $this->caijieary($this->arycont).'">' . $page . '</a>';
                }else{
                    break;
                }
            }else{
                if($page > 0 ){
                    $linkpag .= '<a class="page_but hover"  style="cursor:not-allowed;">' . $page . '</a>';
                }
            }
        }
        return $linkpag;
