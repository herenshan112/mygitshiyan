...
class News extends Controller
{
	...
	*/
	public function index(){
    	...
    	$tyid=post_get('tyid')?post_get('tyid'):3;
    	...
    	//头条
    	$top_list=$conm->index_prolist($tyid,1,2);
    	$this->assign('top_list',$top_list);
        if($top_list){
            $list=$conm->cont_list($tyid,$tyset['pty_pgsum'],$top_list[0]['cn_id']);
        }else{
	...
	*/
	public function details(){
        ...
        if($prourl){
            $this->assign('prourl','<a href="'.url('news/details',array('id'=>$prourl['cn_id'])).'">'.$prourl['cn_title'].'</a>');
        }else{
        ...
        if($nexurl){
            $this->assign('nexurl','<a href="'.url('news/details',array('id'=>$nexurl['cn_id'])).'">'.$nexurl['cn_title'].'</a>');
        }else{
