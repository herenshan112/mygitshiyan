...
class Video extends Controller
{
	...
	*/
	public function index(){
    	...
    	//新闻类别
    	$protym=new Pagetype();
    	$this->assign('protym',$protym->cont_tylists(17));



    	$tyid=post_get('tyid')?post_get('tyid'):17;
	...
	*/
	public function details(){
    	...
    	//产品类别
    	$protym=new Pagetype();
    	$this->assign('protym',$protym->cont_tylist(17));
    	$id=post_get('id');
    	if($id == ''){
    		echo '参数错误！';
    		return;
        ...
        if($prourl){
            $this->assign('prourl','<a href="'.url('voide/details',array('id'=>$prourl['cn_id'])).'">'.$prourl['cn_title'].'</a>');
        }else{
        ...
        if($nexurl){
            $this->assign('nexurl','<a href="'.url('voide/details',array('id'=>$nexurl['cn_id'])).'">'.$nexurl['cn_title'].'</a>');
        }else{
