...
class Place extends Controller
{
	...
	*/
	public function index(){
    	...
    	//新闻类别
    	$protym=new Pagetype();
    	$this->assign('protym',$protym->cont_tylists(23));



    	$tyid=post_get('tyid')?post_get('tyid'):23;
	...
	*/
	public function details(){
    	...
    	//产品类别
    	$protym=new Pagetype();
    	$this->assign('protym',$protym->cont_tylist(23));
    	$id=post_get('id');
    	if($id == ''){
    		echo 'Error';
    		return;
        ...
        if($prourl){
            $this->assign('prourl','<a href="'.url('place/details',array('id'=>$prourl['cn_id'])).'">'.$prourl['cn_title'].'</a>');
        }else{
        ...
        if($nexurl){
            $this->assign('nexurl','<a href="'.url('place/details',array('id'=>$nexurl['cn_id'])).'">'.$nexurl['cn_title'].'</a>');
        }else{
            $this->assign('nexurl','<a href="#">Nill</a>');
        }
